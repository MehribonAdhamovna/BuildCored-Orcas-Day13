...
# TODO #2: Added a 4th source — browser bookmarks, VS Code workspace,
# Spotify plays, calendar events, etc.

def get_running_apps():
    """Returns a list of unique active process names."""
    try:
        # 'ps' works on macOS/Linux. For Windows, you'd use 'tasklist'
        cmd = ["ps", "-Ao", "comm", "-r"] if sys.platform != "win32" else ["tasklist"]
        r = subprocess.run(cmd, capture_output=True, text=True, timeout=5)
        # Clean up the output to get just the app names
        procs = list(set(r.stdout.strip().split("\n")[1:15]))
        return [p.split('/')[-1] for p in procs] 
    except: return []
...
DEBRIEF_PROMPT = """Analyze this developer's telemetry. 
Output EXACTLY 5 lines. No intro, no conversational filler, no markdown bolding.

FORMAT:
BUILT: [1 sentence summary of work]
BROKE: [1 specific error or hurdle identified]
LEARNED: [1 technical insight or discovery]
PATTERN: [The recurring theme of these logs]
NEXT: [The logical immediate next task]
...
console.print(f"  Active Apps:    {len(apps)}")

if apps:
    data.append("ACTIVE PROCESSES:\n" + "\n".join(apps))
